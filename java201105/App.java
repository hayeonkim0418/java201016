...
//게시물번호 , 회원번호 같이 다ㅇ뤄야
						...
						Like rst = likeDao.getLikeMemberIdAndArticleId(target.getId(), LoginedMember.getId());
						
						if(rst == null) {
							Like like = new Like(LoginedMember.getId(), target.getId());
			...
			if(s.equals("article sort")) {
				//sort는 내부적으로 정렬이 안되어있어서 comparator 필요 -> implements 해야함
				//implements -> 내가 정해놓은 메서드를 실현해야함
				//테스트 데이터에 조회수 추가해서 구현
			}
